...
## v3.1 — 섹션별 색상 + 로고 텍스트화 (2026-05-26)

사용자 피드백: "로고 없애고 글씨만 / 카드 여러 색상으로 시각적 분리 / 홈은 살짝 다른색 포인트". Linear/Stripe/Modal/Vercel/Anthropic 벤치마킹.

- **로고**: Lightning 아이콘 제거. `Clone`**`P`**`ilot` 텍스트, 가운데 P만 amber (`var(--brand) = #e0a458`)로 포인트.
- **홈 subtle accent**: hero 배지 dot + form 헤더 dot + 카드 헤더 dot 모두 amber (이전 흰색). 나머지 표면은 그대로 monochrome.
- **카드 섹션별 의미론 색상** (Stripe API docs 톤 — desaturated, eyebrow + 1px 좌측 rail에만 적용, 큰 배경 안 칠함):
  - amber → verdict
  - red → funnel ladder · red flags · top risk
  - blue → market reality
  - green → revenue forecast · green flags
  - yellow → insider tips
  - cyan → build path
  - violet → external footprint
  - orange → description forensics
  - gray → comments · related · raw signals
- `globals.css`에 `--sec-*` 9개 추가. `UrlAnalysisForm.tsx`의 `Section` 컴포넌트가 `tone` prop으로 색상 매핑.
- Verdict box: amber 35% mix border + amber 6% mix background (`color-mix(in oklab, ...)`).
- 검증: 8개 섹션 eyebrow 색상 inspect로 확인 — rgb(214,163,90) amber, rgb(212,106,106) red, rgb(111,158,216) blue, rgb(116,192,143) green, rgb(216,194,93) yellow, rgb(108,199,212) cyan, rgb(160,126,212) violet, rgb(214,146,90) orange.

스크린샷: `v10-hero-clean.png`, `v10-card-top-color.png`, `v10-card-mid-colors.png`.
